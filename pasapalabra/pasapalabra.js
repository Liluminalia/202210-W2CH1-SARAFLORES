const pasapalabra = () => {
    ...
    let firstRound = () => {
        for (i = 0; i < questions.length; i++) {
            ...
            if (Answer === null) {
                ...
                alert(
                    'Has acertado ' +
                        corrects.length +
                        ' respuestas! pero no has terminado el juego, por lo que no puedes entrar en el ranking :(          BYE!'
                );
        ...
        failed = questions.filter(
            (comparedAnswer) => comparedAnswer.status === 3
        );
        pasapalabra = questions.filter(
            (comparedAnswer) => comparedAnswer.status === 2
        );
        corrects = questions.filter(
            (comparedAnswer) => comparedAnswer.status === 1
        );
    };
    let pasapalabraBucle = () => {
        do {
            ...
            pasapalabra = pasapalabra.filter(
                (comparedAnswer) => comparedAnswer.status === 2
            );
